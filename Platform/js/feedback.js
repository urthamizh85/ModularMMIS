...
(function(window, document, undefined) {
	...
	}, nextButton, H2C_IGNORE = "data-html2canvas-ignore", currentPage, modalBody = document.createElement("div");
	...
	window.Feedback = function(options) {
		...
		// default properties
		options.label = options.label || '<i aria-hidden="true" class="fa fa-check"></i>';
		...
		options.messageSuccess = options.messageSuccess || "Your solution was sent succesfully.";
		options.messageError = options.messageError || "There was an error sending your feedback to the server.";

		if (options.pages === undefined) {
			options.pages = [new window.Feedback.Form(), new window.Feedback.Screenshot(options), new window.Feedback.Review()];
		}

		var button, modal, currentPage, glass = document.createElement("div"), returnMethods = {
			...
			open : function() {
				...
				var a = element("a", "×"), modalHeader = document.createElement("div"),
				// modal container
				...
				nextButton.onclick = function() {
					...
					if (currentPage === len) {
						...
						// add page DOM to modal
						modalBody.appendChild(options.pages[currentPage++].dom);
						$('.feedback-review').prepend($('#userDetails_html').html());
						$('.feedback-review').append('<img src="assets/img/whole.png" width="520" style="margin-top: 48px; margin-left: 80px;" />');
						$('#feedback-highlight-close, .feedback-highlight-element, #feedback-highlight-container').remove();
				...
				$(modalFooter).append('<button id="print" class="feedback-btn btn-refresh">Print</button>');
				$(document).off("click", "#print").on("click", "#print", function(e) {
				...
				modal.setAttribute(H2C_IGNORE, true);
				// don't render in html2canvas

				/*
				 html2canvas(document.getElementById('basePlatform'), {
				...
				modal.style.transform = "translateY(-900px)";
				setTimeout(function() {
					nextButton.click();
		...
		button.className = "feedback-btn feedback-bottom-right btn btn-green";
	...
	window.Feedback.Screenshot.prototype.start = function(modal, modalHeader, modalFooter, nextButton) {
		...
		if (this.h2cDone) {
			...
			var $this = this, feedbackHighlightElement = "feedback-highlight-element", dataExclude = "data-exclude";
			...
			this.mouseMoveEvent = function(e) {
				...
				if (e.target !== previousElement) {
					...
					timer = window.setTimeout(function() {
						var bounds = getBounds(previousElement), item;
			...
			}, blackoutButton = element("a", "Blackout"), highlightButton = element("a", "Highlight"), previousElement;
